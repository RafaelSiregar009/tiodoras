...
// nasabahJatuhTempoPetugasProvider di nasabah_provider.dart) — tidak perlu
// ctrl+r atau refresh manual lagi, termasuk saat sudah dibuild jadi APK.
// Sekaligus dipecah jadi 2 bagian: yang SUDAH lewat jatuh tempo, dan yang
// AKAN jatuh tempo dalam 0-2 hari (peringatan H-2) supaya petugas bisa
// mengingatkan nasabah SEBELUM telat.
class PetugasJatuhTempo extends ConsumerWidget {
  const PetugasJatuhTempo({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final async = ref.watch(nasabahJatuhTempoPetugasProvider);

    return Scaffold(
      appBar: AppBar(title: const Text('Nasabah Jatuh Tempo')),
      body: async.when(
  ...
  Widget build(BuildContext context) {
    return NasabahCard(
      ...
      onTap: () => Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) =>
              NasabahDetailScreen(nasabah: nasabah, showAksi: true),
        ),
      ),
    );
  }
